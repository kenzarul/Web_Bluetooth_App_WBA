...
class UsbDfuDevice {
    ...
    dfuStatusHandler = (status) => {
        console.log(status);
    }

    dfuProgressHandler = (value) => {
        console.log(value);
    }
    ...
    async clearStatus() {
        ...
        try {
            const DFU_REQUEST_TYPE = 'class';
            const DFU_RECIPIENT = 'interface';
            const DFU_CLRSTATUS_REQUEST = this.dfuRequest.DFU_CLRSTATUS;
            console.log(`Sending DFU_CLRSTATUS with requestType: ${DFU_REQUEST_TYPE}, recipient: ${DFU_RECIPIENT}, request: ${DFU_CLRSTATUS_REQUEST}`);
            console.log("Attempting to clear DFU status");
    
            // Issue the status clear command
            let result = await this.device.controlTransferOut({
                requestType: DFU_REQUEST_TYPE,
                recipient: DFU_RECIPIENT,
                request: DFU_CLRSTATUS_REQUEST,
                value: 0,
                index: 0 // Make sure this is the correct interface number
            }, undefined);
    
            console.log("Result of DFU_CLRSTATUS:", result);
    
            // If the result is not 'ok'
            if (result.status !== 'ok') {
                console.error("DFU_CLRSTATUS returned non-ok status:", result.status);
                throw new Error("Error: Couldn't clear status, result status: " + result.status);
            }
        }
        // Catch errors
        catch (error) {
            console.error("Error during DFU_CLRSTATUS:", error);
    ...
    async runUpdateSequence(fileArr, flashSizeStr, pageSizeStr) {
        ...
        try {
            ...
            // Update the state
            this.dfuStatusHandler("Connecting");

            // Connect
            await this.connect();

            // Update the state
            this.dfuStatusHandler("Erasing");

            // Erase the chip
            await this.erase();

            // Update the state
            this.dfuStatusHandler("Programming");

            // Program the chip with the binary array
            await this.program(fileArr);

            // Update the state
            this.dfuStatusHandler("Booting");

            // Detach the device
            await this.detach();

            // Update the state
            this.dfuStatusHandler("Disconnecting");
